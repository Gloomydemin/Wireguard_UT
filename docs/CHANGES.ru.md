...
- `Vpn._load_profiles()` — запускает миграцию секретов (если нужно).
- `Vpn._write_profile()` — удаляет `private_key` перед записью `profile.json`.
- `Vpn._migrate_profile_secret(profile_name, data)` — переносит legacy‑ключ в зашифрованное хранилище.
...
- `Vpn.list_profiles()` — использует кеш ключей (быстрее список).
...
- Добавлено поле `PreUp` и сохранение через `save_profile`.
